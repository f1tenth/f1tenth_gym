...
def make_reset_fn(type: str, track: Track, num_agents: int, **kwargs) -> ResetFn:
    if type == "grid_static":
        ...
        return AllTrackResetFn(track=track, num_agents=num_agents, shuffle=False, **kwargs)
    elif type == "random_random":
        return AllTrackResetFn(track=track, num_agents=num_agents, shuffle=True, **kwargs)
    else:
